-- Baseline DDL of PIEDMONT_MCP procedures (CREDIT_MEMO_DB.CREDIT_RISK) captured 2026-06-16T01:48:17Z
-- Captured before rewriting EXECUTE AS OWNER -> EXECUTE AS CALLER for per-role masking. Do not edit; reference only.

-- ===== FIND_OBLIGOR(VARCHAR) =====
...
-- ===== GET_RISK_RATING_TREND(NUMBER,NUMBER) =====
...
-- ===== GET_REVOLVER_USAGE_TREND(NUMBER,NUMBER) =====
  ...
  res := (
    ...
    SELECT OBJECT_CONSTRUCT(
      ...
      ''series'', ARRAY_AGG(OBJECT_CONSTRUCT(''period'', as_of_date, ''obligation_number'', obligation_number,
                          ''commitment'', commitment_amount, ''drawn'', drawn_amount,
                          ''available'', available_amount, ''utilization_pct'', utilization_pct))
...
-- ===== GET_BALANCE_TREND(NUMBER,NUMBER) =====
...
BEGIN
  res := (
    ...
    SELECT OBJECT_CONSTRUCT(
      ...
      ''series'', ARRAY_AGG(OBJECT_CONSTRUCT(''period'', as_of_date, ''ledger_balance'', ledger_balance, ''collected_balance'', collected_balance))
                  WITHIN GROUP (ORDER BY as_of_date),
      ''summary'', OBJECT_CONSTRUCT(
        ''start_balance'', MIN_BY(ledger_balance, as_of_date),
        ''end_balance'',   MAX_BY(ledger_balance, as_of_date),
        ''min_balance'',   MIN(ledger_balance),
        ''change'',        MAX_BY(ledger_balance, as_of_date) - MIN_BY(ledger_balance, as_of_date),
...
-- ===== GET_PAYMENT_HISTORY(NUMBER,NUMBER) =====
...
BEGIN
  res := (
    ...
    SELECT OBJECT_CONSTRUCT(
      ...
      ''series'', ARRAY_AGG(OBJECT_CONSTRUCT(''period'', due_date, ''scheduled_total'', scheduled_total,
                          ''paid_total'', paid_total, ''worst_dpd'', worst_dpd, ''facilities_past_due'', facilities_past_due))
