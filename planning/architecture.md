...
- [~] **AFS live mode** (`infra/afs/go-live.sh`): flips `AFS_FIXTURE_MODE=false` + injects `AFS_USERNAME`/`AFS_PASSWORD` from Secrets Manager `mcpgateway/afs-vision` as runtime env vars (no image rebuild). Working: `afs_show_summary` returns "(live)". **Blocked:** tools that call the live AFS Vision API return `fetch failed` — the secret has **no base URL**, so the app hits an unreachable default. Need the **AFS Vision sandbox base URL** + the env var name the app reads (private repo `Devinboden/AFS-mcp-v2`).
  - **Gotcha (fixed in the script):** `update-agent-runtime` is a full replace — omitting `authorizerConfiguration` **drops the runtime's Cognito JWT authorizer** (reverts to IAM), breaking the gateway's bearer-token calls (`Authorization error when sending message`). The script now always replays the customJWTAuthorizer.
